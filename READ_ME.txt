...
2. lottery.py
 ...
 current date and time and also on an optional
	supplied date.

3. lru.py
   
	In this code snippet LRU algorithm is simulated using a temporary dictionary as cache. Every entry into this dictionary is 
	visualised as a page.The target is to keep frequently used pages(entries) at right of dictionary and least recently used to
	the rear. Such that in case of dictionary reaching
   its max size, rear most entry is deleted making space for recent entry
	at front.
